...
**`sagelite`** is a zero-setup, self-contained, and 100% native portable SageMath distribution for Linux.
...
* **Self-Locating Fixed-Target Entrypoint:** Zero-overhead `./sage` entrypoint that resolves installation path, runs `conda-unpack` once upon initial launch at target destination, and sanitizes environment variables.
...
### 2. Hermetic Environment Isolation & Toolchain Activation
* The runtime wrapper **MUST** prevent host Python and library pollution. Always enforce:
  * `unset PYTHONPATH` and `unset PYTHONSTARTUP`
  * `PYTHONNOUSERSITE=1` (blocks `~/.local/lib/python*`)
  * `PYTHONHOME="$DIR"`
  * `SAGE_ROOT="$DIR"` and `SAGE_LOCAL="$DIR"`
  * Source `$DIR/etc/conda/activate.d/*.sh` to inject compiler `--sysroot` and toolchain flags.
...
### 5. Deterministic Fixed-Target Packaging
...
* Dynamic prefix initialization is automatically performed once on the first user execution at the target destination via `./sage`.
...
### 4.2. Local Verification Testing

```bash
# 1. Extract directly to target test directory:
mkdir -p /tmp/test-sagelite
tar --zstd -xf artifacts/sagemath-portable-x86_64.tar.zst -C /tmp/test-sagelite

# 2. Run the automated 5-tier verification suite (~30s):
./scripts/test.sh /tmp/test-sagelite/sage

# 3. (Optional) Run full official Sage doctest suite:
...
* **Fixed-Target Initialization:** Prefix initialization (`conda-unpack`) is run once upon initial launch and guarded by the sentinel file `$DIR/.unpacked`. If moved post-unpack, the safety guard halts with an explicit instruction to re-extract directly to the target path.
* **CLI Convenience Routing:** The `./sage` wrapper intercepts sub-commands to provide direct access to embedded utilities:
  * `./sage -pip ...` $\rightarrow$ `$DIR/bin/python -m pip ...`
...
3. [ ] **Fixed-Target Verification:** Tested extraction directly to target path and verified clean first-run initialization.
